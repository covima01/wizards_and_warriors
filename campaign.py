...
def two_enemies(player, enemy1, enemy2):
    if isinstance (player, Warrior) and isinstance (enemy1, Ogre) and isinstance (enemy2, Ogre):
        while player.health > 0 and enemy1.health > 0 or enemy2.health > 0:
            time.sleep(0.5)
            print(BORDER)
            print(f"{player.name}-- Health: {player.health} , Endurance: {player.endurance} // {enemy1.name}-- Health: {enemy1.health} : {enemy2.name}-- {enemy2.health}".center(70))
            ...
            if target == "2":
                ...
                while selection != "1" and selection != "2" and selection != "3":
                    print("Unrecognized selection. Please choose 1) Light Swing or 2) Heavy Swing 3) Meditate")
                    selection = input(f"Which action will {player.name} take?  {player.name} has {player.health} health. {enemy2.name} has {enemy2.health} health.\n\n 1) Light Swing\n 2) Heavy Swing\n 3) Meditate")
                if selection == "1":
                    attack = random.randint(0,8)
                    if attack <= 7:
...
def three_enemies(player, enemy1, enemy2, enemy3):
    if isinstance (player, Warrior) and isinstance (enemy1, Ogre) and isinstance (enemy2, Ogre) and isinstance(enemy3, Ogre):
        while player.health > 0 and enemy1.health > 0 or enemy2.health > 0 or enemy3.health > 0:
            time.sleep(0.5)
            print(BORDER)
            print(f"{player.name}-- Health: {player.health} , Endurance: {player.endurance} // {enemy1.name}-- Health: {enemy1.health} : {enemy2.name}-- Health: {enemy2.health} : {enemy3.name}-- Health: {enemy3.health}".center(70))
            target = input(f"Who will {player.name} attack?\n 1) {enemy1.name} \n 2) {enemy2.name} \n 3) {enemy3.name}")
            print(BORDER)
            ...
            if target == "3":
                ...
                while selection != "1" and selection != "2" and selection != "3":
                    print("Unrecognized selection. Please choose 1) Light Swing or 2) Heavy Swing 3) Meditate")
                    selection = input(f"Which action will {player.name} take?  {player.name} has {player.health} health. {enemy3.name} has {enemy3.health} health.\n\n 1) Light Swing\n 2) Heavy Swing\n 3) Meditate")
                if selection == "1":
                    attack = random.randint(0,8)
                    if attack <= 7:
